...
execute as @e[tag=stmp] at @s run tp @s ~ ~ ~ ~0.0 ~
execute as @e[tag=stmp0] at @s run tp @s ~ ~ ~ ~180.0 ~
tp @e[tag=stmp0] ~ ~ ~
tp @e[tag=stmp] ~ ~ ~
execute as @e[tag=stmp0] at @s run tp @s ^ ^ ^5.0
execute as @e[tag=stmp] at @s run tp @s ^ ^ ^5.0
summon item ~ 180.0 ~ {Item:{id:"minecraft:glass",Count:1b,tag:{line:[[0.0d,0.0d],[0.0d,0.0d]]}},Tags:["stmp2","result","par"],NoGravity:true}
