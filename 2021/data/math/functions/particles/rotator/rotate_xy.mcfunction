...
execute store result score y0 mpar_temp run data get entity @e[tag=stmp,limit=1] Pos[1] 10000
tag @e remove stmp

#获取当前坐标
execute store result score x1 mpar_temp run data get entity @s Pos[0] 10000
execute store result score y1 mpar_temp run data get entity @s Pos[1] 10000
...
execute store result entity @s Pos[1] double 0.0001 run scoreboard players operation y2 mpar_temp += y0 mpar_temp