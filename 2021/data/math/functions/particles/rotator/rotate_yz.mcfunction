...
execute store result score x0 mpar_temp run data get entity @e[tag=stmp,limit=1] Pos[1] 10000
execute store result score y0 mpar_temp run data get entity @e[tag=stmp,limit=1] Pos[2] 10000
tag @e remove stmp

#获取当前坐标
execute store result score x1 mpar_temp run data get entity @s Pos[1] 10000
...
execute store result entity @s Pos[1] double 0.0001 run scoreboard players operation x2 mpar_temp += x0 mpar_temp
